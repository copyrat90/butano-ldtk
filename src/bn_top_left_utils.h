...
{
    return fixed_point(to_top_left_x(position.x(), width), to_top_left_y(position.y(), height));
}
...
[[nodiscard]] constexpr fixed_point from_top_left_position(const fixed_point& top_left_position, int width, int height)
{
    return fixed_point(from_top_left_x(top_left_position.x(), width), from_top_left_y(top_left_position.y(), height));
}

[[nodiscard]] constexpr fixed_point from_top_left_position(const fixed_point& top_left_position, const size& dimensions)
{
